...
loop do
  if guesses > max
    ...
    if guesses == 1
      image_status = File.read("img/guillotined/minor.txt").strip

      puts image_status
    elsif guesses == 2
      image_status = File.read("img/guillotined/mild.txt").strip

      puts image_status
    elsif guesses == 3
      image_status = File.read("img/guillotined/moderate.txt").strip

      puts image_status
    elsif guesses == 4
      image_status = File.read("img/guillotined/severe.txt").strip

      puts image_status
    elsif guesses == 5
      image_status = File.read("img/guillotined/critical.txt").strip
